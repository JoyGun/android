...
public class PointDetector {
    ...
    public static PointDetector create(Activity activity, Classifier.Model model, Classifier.Device device, int numThreads)
            throws IOException {
            return new PointDetector(activity, device, numThreads);
    }
     ...
     * @param numThreads
     */
    protected PointDetector(Activity activity, Classifier.Device device, int numThreads) throws IOException {
        ...
        Log.i("ocr sever","create model1");
    }

    private String getModelPath() {
        return "tf_detect_model_v12_0921.mp3";
    }
...
//        perspectiveChange(0,0,null);
        ...
        if(isRect(points)){
            ...
            if(regSide == IdCardRegConstant.FRONT_SIDE){
                List<RangesArray> rangesArrays = extractTextArea2(bitmap1,0,0,(int)(0.4*bitmap1.getWidth()),bitmap1.getHeight());
                //画出文本区域
                //drawTextRect(bitmapDraw,rangesArrays);
                ...
                //背面切片
                List<RangesArray> rangesArrays = extractTextArea2(bitmap1,(int)(0.16*bitmap1.getWidth()),(int)(0.5*bitmap1.getHeight()),(int)(0.84*bitmap1.getWidth()),(int)(0.5*bitmap1.getHeight()));
                //画出文本区域
                //drawTextRect(bitmap1,rangesArrays);
                for(RangesArray rangesArray:rangesArrays){
...
//            }
     ...
     * @return
     */
    private List<RangesArray> extractTextArea2(Bitmap bitmap, int cannyXCoordinate, int cannyYCoordinate, int cannyXWidth, int cannyYHeight) {
        int minW = 10;
        int maxW = 30;
        int h = bitmap.getHeight();
        int w = bitmap.getWidth();
        List<Integer> whiteCntList = new ArrayList();
        List<RangesArray> rangesArrayList = new ArrayList<>();
